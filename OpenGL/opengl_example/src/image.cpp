...
ImageUPtr Image::Create(int width, int height, int chaanelCount){
    auto image = ImageUPtr(new Image());
    if(!image->Allocatie(width, height, chaanelCount))
        return nullptr;
    return std::move(image);
}
...
bool Image::Allocatie(int width, int height, int channelCount){
    m_width = width;
    m_height = height;
...
bool Image::LoadWithStb(const std::string& filepath){
    ...
    if(!m_data){
        SPDLOG_ERROR("filaed to load image: {}", filepath);
        return false;
    }
    return true;
}
